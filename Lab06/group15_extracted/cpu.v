...
module cpu (PC, INSTRUCTION, CLK, RESET, READ, WRITE, ADDRESS, WRITEDATA, READDATA, BUSYWAIT);
    ...
    // =========================================================================
    // FIX FOR IMMEDIATE VALUE TIMING ALIGNMENT
    // =========================================================================
    // Previously, INSTRUCTION[7:0] was fed directly into the MUX with zero delay.
    // This caused the ALU to bypass the #1 Control Unit decode delay and execute 
    // too early (in the 3-4ps block) for immediate instructions like lwi/swi.
    // To fix this, we created an intermediate wire 'immediate_val' with a #1 
    // delay to perfectly simulate the decode extraction time.
    wire [7:0] immediate_val;
    assign #1 immediate_val = INSTRUCTION[7:0]; 

    // Operand 2 multiplexer (Op2Select)
    // We now feed the delayed 'immediate_val' into the multiplexer instead of 
    // the raw INSTRUCTION wire. This forces the ALU to wait until the 4ps mark 
    // before calculating, perfectly aligning with Register Direct instructions.
    assign alu_in2 = (op2_select == 2'b00) ? out2 :                 // if op2_select is 00, select out2
                 ...
                 (op2_select == 2'b10) ? immediate_val :            // selects the delayed immediate value!
                 8'h00;                                             // default case
    // =========================================================================
