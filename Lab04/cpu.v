// CO2070 Computer Architecture- Lab 4
...
module control_unit (opcode, RegWrite, ALUOP, Op2Select, JUMP, BRANCH);   // LAB 04 MODIFICATION: added JUMP and BRANCH ports
...
module cpu (PC, INSTRUCTION, CLK, RESET);
    ...
    wire jump, branch, zero;                // LAB 04 MODIFICATION: Declare control and status wires

    
    assign #1 pc_next = PC + 32'h4;         // Increment PC by 4 to point to the next instruction (assuming 4-byte instruction width)

    // LAB 04 MODIFICATION: Branch/Jump Target Adder (latency #2)
    ...
    // LAB 04 MODIFICATION: PC Source Multiplexer
    ...
    control_unit cu_inst (.opcode(INSTRUCTION[31:24]), .RegWrite(reg_write), .ALUOP(alu_op), .Op2Select(op2_select), .JUMP(jump), .BRANCH(branch));       // LAB 04 MODIFICATION: added JUMP and BRANCH
