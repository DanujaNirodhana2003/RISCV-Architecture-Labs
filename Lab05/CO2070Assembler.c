...
This program can be used to convert manually-written textual assembly programs into machine code for CO2070 laboratory exercises 3 and following ones.
...
Compiling the program	: gcc CO2070Assembler.c -o CO2070Assembler
Using the assembler	: ./CO2070Assembler <your_assembly_file_name> (e.g. ./CO2070Assembler program.s)
...
int main(int argc, char *argv[])
{
	...
	*************************************************************************/
	...
	char *op_bne = "00001100"; // LAB 4.5 MODIFICATION
	char *op_sll = "00001101"; // LAB 4.5 MODIFICATION
	...
	while (fgets(line, LINE_SIZE, fi) != NULL) // Read a line from the input .s file
	{
		...
		*******************************************************************/
		...
		while (in_token != NULL)
		{
			...
			else if (strcasecmp(in_token, "bne") == 0) // LAB 4.5 MODIFICATION
				strcpy(out_token, op_bne);             // LAB 4.5 MODIFICATION
			else if (strcasecmp(in_token, "sll") == 0) // LAB 4.5 MODIFICATION
				strcpy(out_token, op_sll);             // LAB 4.5 MODIFICATION
